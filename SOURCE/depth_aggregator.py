...
def depth_aggregator(in_file=None, depth_array_str=None, out_file=None,
    ...
    try:
        ...
        try:
            ...
            time.sleep(sleep_time)
            print(' Error. Depth dimension not found in input file.', file=sys.stderr)
            time.sleep(sleep_time)
            print(' -------------------------')
            return
    ...
    for variable_name in out_fixed_variables:
        ...
        if not in_fixed_variable.shape:
    ...
    for variable_name in out_variables:
        ...
        if depth_dimension_name in in_variable.dimensions:
            ...
            out_variable_attributes = \
                [attribute for attribute in in_variable.ncattrs() if attribute not in '_FillValue']
            out_variable.setncatts({attribute: in_variable.getncattr(attribute)
                                    for attribute in out_variable_attributes})
            out_variable.valid_min = np.float32(np.min(out_variable_data))
            out_variable.valid_max = np.float32(np.max(out_variable_data))
        else:
            if verbose:
                print(' Copying variable ' + variable_name)
            # Create output variable
            out_variable = out_data.createVariable(variable_name, in_variable.datatype,
                                                   dimensions=in_variable.dimensions)

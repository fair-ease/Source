...
# Functional version
def unique_values_nc_variable(in_file=None, in_variable_standard_name=None, verbose=True):
    if __name__ == '__main__':
        return
    if verbose:
        print(' -------------------------' + ' ' + __file__ + ' -------------------------')
        print(' Script to compute variable unique values over time dimension.')
        print(' -------------------------')
    ...
    if np.ma.is_masked(in_variable_data):
        in_variable_data = in_variable_data.data
    in_variable_unique_values = len(np.unique(in_variable_data))
    if verbose:
        print(' Unique values of ' + in_variable_name + ' = ' + str(in_variable_unique_values))

    in_data.close()

    return in_variable_unique_values
...
if os.path.basename(sys.argv[0]) == os.path.basename(__file__):
    ...
    in_variable_unique_values = unique_values_nc_variable(in_file, in_variable_standard_name, verbose)
    print(in_variable_unique_values)

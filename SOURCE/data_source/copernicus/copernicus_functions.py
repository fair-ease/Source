...
def readIndexFileFromCWD(path2file, targeted_bbox, med_poly):
    ...
    if targeted_bbox != None:
        ...
        for chunk in chunks:
            ...
            chunk['poligonOverlap'] = chunk.apply(poligonOverlap,targeted_bbox=targeted_bbox, med_poly=med_poly, axis=1)
...
def poligonOverlap(row, targeted_bbox,  med_poly):
    from shapely.geometry import box, Point, Polygon
    # Checks if a file contains data in the specified area (targeted_bbox)
    result = False
    try:
        ...
        geospatial_lon_min = float(row['geospatial_lon_min'])
        geospatial_lon_max = float(row['geospatial_lon_max'])
        targeted_point = Point(geospatial_lon_min,geospatial_lat_min)
        local_targeted_box = box(geospatial_lon_min,geospatial_lat_min,geospatial_lon_max,geospatial_lat_max)
      
#        if targeted_point.within(med_poly):  # check other rules on https://shapely.readthedocs.io/en/stable/manual.html
        #if targeted_point.within(targeted_bbox):  # check other rules on https://shapely.readthedocs.io/en/stable/manual.html
        #if local_targeted_box.within(med_poly):  # check other rules on https://shapely.readthedocs.io/en/stable/manual.html
        #if local_targeted_box.intersects(med_poly):  # check other rules on https://shapely.readthedocs.io/en/stable/manual.html
        if targeted_point.within(med_poly):  # check other rules on https://shapely.readthedocs.io/en/stable/manual.html
            result = True
#        lon_lat_list = [
#          [float(row['geospatial_lon_min']),float(row['geospatial_lat_min'])],
#          [float(row['geospatial_lon_min']),float(row['geospatial_lat_max'])],
#          [float(row['geospatial_lon_max']),float(row['geospatial_lat_max'])],
#          [float(row['geospatial_lon_max']),float(row['geospatial_lat_min'])]
#        ]
#        row_geom_poly = Polygon(lon_lat_list)
...
def getIndexFilesInfo(dataset, local_dir, targeted_bbox, med_poly):
    ...
    indexPlatform = readIndexFileFromCWD(path2file, None, None)
    ...
    #data_top = netcdf_collections.keys()
    #print(data_top)
    # 3) creating new columns: derived info
    #netcdf_collections['parameters'] = netcdf_collections['parameters']
    #netcdf_collections['poligonOverlap'] = netcdf_collections['poligonOverlap']
